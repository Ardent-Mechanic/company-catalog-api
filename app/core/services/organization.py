...
from app.core.schemas import CombineOrgFilterPagination, OrganizationOut
...
class OrganizationService:
    ...
    async def search(
        ...
        items, total = await self.repo.find_many(filter_, offset, limit, sort)
    ...
    async def find_nearby(self, lat: float, lon: float, radius_km: float, limit: int = 20) -> dict:
    ...
    async def find_in_square(
        self, lat1: float, lon1: float, lat2: float, lon2: float, limit: int = 20
    ) -> dict:
        orgs, total = await self.repo.find_in_square(
            lat1=lat1, lon1=lon1, lat2=lat2, lon2=lon2, limit=limit
        )

        if not orgs:
            raise NotFoundError("Organizations not found in the specified square")
